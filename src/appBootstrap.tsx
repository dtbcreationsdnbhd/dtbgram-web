...
// Restores the real head links removed from `index.html` so nothing app-related loads before unlock
function restoreRealHead() {
  const { head } = document;
  const publicPath = './';

  function appendIcon(attrs: Record<string, string>) {
    const link = document.createElement('link');
    Object.entries(attrs).forEach(([key, value]) => link.setAttribute(key, value));
    head.appendChild(link);
  }

  appendIcon({ rel: 'apple-touch-icon', sizes: '180x180', href: `${publicPath}${import.meta.env.TG_APPLE_ICON}.png` });
  appendIcon({
    rel: 'icon', type: 'image/png', sizes: '32x32', href: `${publicPath}favicon-32x32.png`,
  });
  appendIcon({
    rel: 'icon', type: 'image/png', sizes: '16x16', href: `${publicPath}favicon-16x16.png`,
  });
  appendIcon({
    rel: 'icon', type: 'image/png', sizes: '192x192', href: `${publicPath}${import.meta.env.TG_MAIN_ICON}.png`,
  });
  appendIcon({ rel: 'icon', href: `${publicPath}favicon.svg`, type: 'image/svg+xml' });
  appendIcon({ rel: 'shortcut icon', href: `${publicPath}favicon.ico`, type: 'image/x-icon' });
  appendIcon({ rel: 'alternate icon', href: `${publicPath}favicon.ico`, type: 'image/x-icon' });

  const manifest = document.createElement('link');
  manifest.setAttribute('rel', 'manifest');
  manifest.setAttribute('id', 'the-manifest-placeholder');
  manifest.setAttribute('href', `${publicPath}${import.meta.env.TG_MANIFEST}`);
...
function removeLockFavicon() {
  document.querySelectorAll('link[data-app-lock-favicon]').forEach((el) => el.remove());
}
